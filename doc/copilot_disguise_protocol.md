...
HTTP/2 Stream 建联后，协议进入底层硬核帧结构通信，用以支持高并发的 TCP/UDP/ICMP 会话承载。为了兼顾协议扩展性与基础的自校验能力，固定头部增加了两字节帧头、两字节通讯链路字段与 2 字节校验和，合计 **16 字节** = magic(2) + link_id(2) + cmd(1) + subcmd(1) + conn_id(4) + len(4) + crc16(2)。

```
通信帧结构 (16 Bytes Header):
┌────────────┬─────────────┬──────────┬─────────────┬──────────────┬──────────┬────────────┬────────────────┐
│ magic (2B) │ link_id (2B)| cmd (1B) │ subcmd (1B) │ conn_id (4B) │ len (4B) │ crc16 (2B) │ payload        │
└────────────┴─────────────┴──────────┴─────────────┴──────────────┴──────────┴────────────┴────────────────┘

magic  — 帧起始同步标识（预留值，例如 0x7E 0x7E，用于校验错位或后续版本演进）
link_id — 通讯链路标识（虚拟链路 ID，固定级联关系标识，取值由主控统一下发；可表示单向或双向链路）
cmd    — 操作类型（一级分类）
subcmd — 该操作的具体子态或扩展参数（未定义时置 0x00）
len    — payload 字节长度（仅包含 payload，不包含固定 16B Header）
crc16  — 针对 Header(除自身外) + Payload 的 CRC16 校验和（用于基础完整性校验）

除保活与全局控制帧外，业务帧必须携带有效 `link_id`；探针仅允许转发主控已下发并授权的 `link_id` 级联关系。
帧解析规则：先读取固定 16B Header，再按 `len` 继续读取 payload（读取长度严格等于 `len`）。

级联转发规则：在同一 `link_id` 对应的固定级联路径上，仅入口节点与出口节点解析 `cmd/subcmd` 并执行业务语义；中间节点不解析命令语义、不解码业务 payload，仅执行帧完整性校验（`magic/len/crc16`）与 `link_id` 授权校验，然后按主控下发的下一跳关系原样转发帧。

方向语义：每个 `link_id` 标识的链路可被主控定义为单向链路或双向链路。双向链路下，正反两个方向共享同一条固定级联关系；单向链路下，去程与回程应视为两条独立的逻辑路径，其路由可不一致，并由主控分别下发对应的 `link_id` 或方向映射关系。

重传职责边界：本协议层不提供分片重组后的自动重传、乱序重排恢复或可靠送达保证；重传机制由底层传输链路（如 TCP）或上层业务协议自行负责。

承载层兼容性：本通讯帧协议为承载无关设计，可运行在 TCP、UDP、HTTP/1.1、HTTP/2、WebSocket 之上；其中当前标准实现与默认生产链路为 HTTPS + HTTP/2 Native Stream。
...
依托 `link_id + conn_id` 组合键，单根物理 HTTP/2 Stream 链路能够同时承载数百条独立的 TCP 逻辑流、UDP 关联和 ICMP 会话。`link_id` 用于隔离主控下发的固定级联关系，`conn_id` 在同一 `link_id` 下进行会话复用（conn_id 空间各协议类型共享，互不干扰）。
